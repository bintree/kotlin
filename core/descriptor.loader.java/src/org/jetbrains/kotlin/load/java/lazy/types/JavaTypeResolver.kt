...
class JavaTypeResolver(
    ...
    override fun findClassifier(qName: String): JavaClassifier? {

        return org.jetbrains.kotlin.utils.findClassifier.time {  typeParameterResolver.resolveTypeParameter(qName) ?: findClass(qName) }
    }

    private fun findClass(qName: String): JavaClassifier? {
        return org.jetbrains.kotlin.utils.findClass.time {
            val fqName = FqName(qName)
            var packageFqName = fqName.parent()
            var nestedFqName = FqName(fqName.shortName().asString())

            while (true) {
                c.components.finder.findClass(ClassId(packageFqName, nestedFqName, false))?.let { return@time it }
